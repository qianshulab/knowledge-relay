...
| 内容检索 | 优先结合 SQLite FTS5 全文匹配、结构化字段与语义召回；运行时未提供 FTS5 时自动使用持久化令牌索引兼容检索 |
...
“检索个人知识”和“知识问答”是两个独立入口：检索用于快速定位可能相关的收藏；知识问答用于跨多篇资料归纳、比较和连续追问。全文索引随内容入库、更新、恢复和删除增量维护，不需要每次搜索重新学习全部资料；管理员可以从任务中心触发一次完整重建来修复旧数据索引。系统会优先使用 SQLite FTS5；若 NAS、Node.js 或其他运行环境未编译 FTS5，会自动切换到持久化令牌扫描，功能保持可用但超大知识库的查询速度会降低。
