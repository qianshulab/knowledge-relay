...
“检索个人知识”和“知识问答”是两个独立入口：检索用于快速定位可能相关的收藏；知识问答用于跨多篇资料归纳、比较和连续追问。全文索引随内容入库、更新、恢复和删除增量维护，不需要每次搜索重新学习全部资料；管理员可以从任务中心触发一次完整重建来修复旧数据索引。应用通过 `better-sqlite3` 自带启用 FTS5 的跨平台 SQLite，不依赖 NAS 或 Node.js 系统 SQLite 的编译选项；只有原生模块损坏等异常情况下才会启用较慢的持久化令牌扫描兜底。
