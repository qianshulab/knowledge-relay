...
知流是一套开源、自托管的知识收件与整理系统。它接收来自网页上传、微信 iLink、统一微信助手和开放 API 的文字、链接及附件，保存原始内容，解析网页、微信公众号文章与常见文档，再将结果整理为可搜索、可阅读、可同步的个人知识资源。
...
| 模块 | 能力 |
|---|---|
| 统一收件 | 支持网页直接上传、RSS/Atom 自动订阅，并接收微信 iLink、统一微信助手与开放 API 提交的文字、链接、图片、语音、视频和文件 |
| 内容解析 | 提取普通网页及微信公众号正文、原始封面和正文图片；解析 PDF、DOCX、XLSX、Markdown、HTML 及带资源的 ZIP，生成可离线阅读的 Markdown 快照 |
| 智能整理 | 生成标题、摘要、内容形态、动态主题、标签、领域、知识点和工具信息 |
| 收件台 | 展示处理状态、失败原因、附件和同步结果，支持重新整理、归档与永久删除 |
| 知识库 | 按内容形态、主题和领域浏览已整理内容，支持智能收藏夹、未读筛选和独立文章阅读页 |
| 阅读与回顾 | 支持正文划线、批注、笔记、重点标记和内容重读提醒，让长期收藏可以持续被重新发现 |
| 任务中心 | 持久化记录整理、图解、来源检查等后台任务；页面关闭或服务重启后仍可查看进度、失败原因和重试结果 |
| 内容检索 | 使用应用内置的 SQLite FTS5 全文匹配，结合结构化字段与语义召回，在当前用户的增量索引中定位和归纳资料 |
| 知识问答 | 支持限定整库、智能集合、筛选条件或单篇资料持续问答，保存会话并提供可点击的资料依据 |
| 智能图解 | 按资料结构选择关系图、流程图、对比图、时间线、时序图、状态图或思维导图；支持缩放、搜索、节点解释与证据查看 |
| 内容质量 | 集中识别解析失败、正文缺失、图片引用失效和内容不完整的问题，支持选择内容后批量重新整理 |
| 内容治理 | 同一链接重复到达时合并记录而不重复整理；保留整理版本、完整性评分和可恢复的历史快照 |
| 数据备份 | 提供个人数据导出、备份文件校验以及 Docker 备份、恢复和回退流程 |
...
4. 在收件台上传一个文档，或发送一条文字和网页链接，确认收件、解析与整理状态。
...
收件台右上角的“添加内容”支持一次上传最多 10 个文件，总大小上限为 100 MB。上传内容与微信附件进入同一套租户隔离、去重、整理、检索和同步流程。

| 格式 | 处理方式 |
|---|---|
| JPG、PNG、GIF、WebP | 交给已配置的视觉模型理解，生成摘要、要点和可阅读说明 |
| PDF | 本地提取分页文本；扫描版 PDF 在文本不足时交给模型补充识别 |
| DOCX | 保留标题、段落、列表、表格和可验证的正文图片 |
| XLSX、CSV、TSV | 按工作表转换为有边界的 Markdown 表格，避免整表挤成普通段落 |
| Markdown、TXT、JSON、YAML、日志 | 保留原始文本和代码结构 |
| HTML | 清理脚本等非正文元素，保留标题、列表、代码块、表格与图片引用 |
| ZIP | 安全检查路径、文件数、解压体积和压缩比；自动识别 README 或 index 主文档 |
| Markdown + 资源 ZIP、HTML + 资源 ZIP | 根据相对路径导入正文图片并改写为本地附件引用，可离线阅读和同步 |

文档正文先由本地确定性解析器生成，即使模型服务暂时不可用也能阅读和检索；AI 负责进一步生成标题、摘要、主题、知识点和问答索引。旧版 `.doc`、`.xls` 二进制文件建议先转换为 DOCX、XLSX。

### 自动来源

“系统设置 → 收件接入 → 自动来源”可以添加 RSS 或 Atom 地址，并设置 15 分钟到 24 小时的检查间隔。系统使用与网页解析相同的网络安全策略拉取订阅，只接收新增条目；首次启用最多导入最近 10 条，避免历史内容瞬间塞满收件台。

每次检查都会进入任务中心并保留结果。来源暂时不可用时会记录最近错误并在后续周期自动重试，已经入库的内容不受停用或删除订阅影响。手动“立即检查”适合验证新来源，不需要刷新页面等待结果。
...
“检索个人知识”和“知识问答”是两个独立入口：检索用于快速定位可能相关的收藏；知识问答用于跨多篇资料归纳、比较和连续追问。全文索引随内容入库、更新、恢复和删除增量维护，不需要每次搜索重新学习全部资料；管理员可以从任务中心触发一次完整重建来修复旧数据索引。应用通过 `better-sqlite3` 自带启用 FTS5 的 SQLite，不依赖 NAS 或 Node.js 系统 SQLite 的编译选项。Docker 发布流程会分别在 amd64、arm64 目标环境中从源码编译原生模块，并在构建和已发布镜像冒烟测试中实际创建 FTS5 索引；验证失败的镜像不会发布。只有原生模块损坏等异常情况下才会启用较慢的持久化令牌扫描兜底。

问答范围可以是整个知识库、一个智能集合、当前筛选结果或阅读中的单篇文章。系统只把范围内已完成整理的知识片段交给模型，回答附带可点击来源；知识库中没有足够依据时会明确说明，不使用网络信息或模型记忆补齐事实。两个入口均为只读，不执行删除、修改、同步或系统命令。

### 阅读、版本与内容完整性

阅读页按照正文结构生成文章目录和阅读进度，代码块、表格、图片及附件引用使用统一的安全渲染规则。正文、整理笔记、延伸整理和原始内容彼此独立，避免派生附件挤占阅读区域。

每次整理完成或恢复旧内容时都会创建版本快照。用户可以查看标题、摘要、正文、分类、主题、知识点和工具字段的历史状态，并把任意版本恢复为一个新的当前版本；恢复不会覆盖或删除既有历史。

内容完整性卡片会核对正文长度、本地图片数量和附件引用，标记正文缺失、内容过短或本地资源失效。相同规范化链接再次到达时只更新重复次数和最近到达时间，不再创建第二条整理任务；阅读页会显示已合并次数。

### 任务中心与运行状态

整理、重新整理、智能图解、自动来源检查和索引维护统一写入持久化任务中心。任务状态不依赖某个弹窗或浏览器页面，关闭页面后再次打开仍会显示当前阶段、进度、起止时间和错误原因；服务重启后未完成任务会按类型恢复或明确标记，避免出现“看起来一直处理中”的假状态。
...
cd /你的部署目录/knowledge-relay
git pull --ff-only
./scripts/update-docker.sh 1.11.0
```
