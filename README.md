...
| 内容检索 | 结合 SQLite FTS5 全文匹配、结构化字段与语义召回，在当前用户的增量索引中查找并归纳相关资料 |
...
“检索个人知识”和“知识问答”是两个独立入口：检索用于快速定位可能相关的收藏；知识问答用于跨多篇资料归纳、比较和连续追问。全文索引随内容入库、更新、恢复和删除增量维护，不需要每次搜索重新学习全部资料；管理员可以从任务中心触发一次完整重建来修复旧数据索引。
