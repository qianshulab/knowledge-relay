...
import { Check, Copy, Download, ExternalLink, FileArchive, Gem, KeyRound, Plus, Trash2, Upload, X } from "lucide-react";
import { api } from "../api";
...
export default function ObsidianPage() {
  ...
  const createTarget = useMutation({ mutationFn: (data: { name: string; folder: string }) => api<SyncTarget>("/api/sync-targets", { method: "POST", body: JSON.stringify({ ...data, primary: !targets.data?.targets.length }) }), onSuccess: (value) => { setCreated(value); notify("同步设备已创建，请立即保存令牌", "success"); void queryClient.invalidateQueries({ queryKey: ["sync-targets"] }); } });
  ...
  async function revoke(id: string) {
    if (!window.confirm("撤销后，这个设备将无法继续同步。确定继续吗？")) return;
  ...
  function submitPlugin(event: FormEvent<HTMLFormElement>) {
    ...
    if (!window.confirm("发布后，所有用户下载到的将是这个插件版本。确定继续吗？")) return;
