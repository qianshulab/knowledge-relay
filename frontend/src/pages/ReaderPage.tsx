...
export default function ReaderPage() {
  ...
  const generateDiagram = useMutation({ mutationFn: (force: boolean) => api<DiagramResponse>(`/api/messages/${encodeURIComponent(messageId)}/diagram`, { method: "POST", body: JSON.stringify({ force }) }), onSuccess: (data) => { queryClient.setQueryData(["diagram", messageId], data); notify("智能图解已生成", "success"); }, onError: (error) => notify(error instanceof Error ? error.message : "图解生成失败", "danger") });
