...
describe("plugin release", () => {
  ...
  it("publishes to the persistent data directory and prevents ambiguous replacements", async () => {
    ...
    const first = archive({ version: "1.4.0" });
    const published = await publishPluginRelease(config, first);
    expect(published).toMatchObject({ available: true, version: "1.4.0", source: "uploaded" });
    ...
    await expect(publishPluginRelease(config, first)).resolves.toMatchObject({ version: "1.4.0" });
    await expect(publishPluginRelease(config, archive({ version: "1.4.0", extra: ["wechat-ilink-inbox-sync/styles.css", "changed"] })))
      .rejects.toMatchObject({ statusCode: 409 });
    await expect(publishPluginRelease(config, archive({ version: "1.2.9" })))
      .rejects.toThrow("不能从 v1.4.0 降级");
  });
