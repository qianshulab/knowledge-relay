...
describe("plugin release", () => {
  ...
  it("publishes to the persistent data directory and prevents ambiguous replacements", async () => {
    ...
    const first = archive({ version: "1.4.1" });
    const published = await publishPluginRelease(config, first);
    expect(published).toMatchObject({ available: true, version: "1.4.1", source: "uploaded" });
    ...
    await expect(publishPluginRelease(config, first)).resolves.toMatchObject({ version: "1.4.1" });
    await expect(publishPluginRelease(config, archive({ version: "1.4.1", extra: ["wechat-ilink-inbox-sync/styles.css", "changed"] })))
      .rejects.toMatchObject({ statusCode: 409 });
    await expect(publishPluginRelease(config, archive({ version: "1.2.9" })))
      .rejects.toThrow("不能从 v1.4.1 降级");
  });
  ...
  it("应用升级后优先提供比历史上传包更新的内置版本", async () => {
    ...
    await expect(resolvePluginRelease(config)).resolves.toMatchObject({ version: "1.4.0", source: "bundled" });
  });
});
