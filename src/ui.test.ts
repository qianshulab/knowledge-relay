...
describe("adminPage", () => {
  ...
  it("个人版不再暴露注册或用户管理，并给手动刷新明确反馈", () => {
    ...
    expect(adminPage).toContain('class="account-card"');
    expect(adminPage).toContain('class="settings-hub"');
    expect(adminPage).toContain('class="capture-flow"');
    expect(adminPage).toContain("知识收件台");
    expect(adminPage).toContain("primaryPage=['wechat','agent','skills'].includes(page)?'settings':page");
    expect(adminPage).toContain("个人工作区");
